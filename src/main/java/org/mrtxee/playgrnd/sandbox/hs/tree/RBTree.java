...
public class RBTree<T extends Comparable<T>> extends BSTree<T> {
  ...
  private void RBTreeInsertionFix(RBTreeNode<T> node) {
    while (node != this.getRoot() && null != node.parent && null != node.parent.parent
        && node.parent.color == RBTreeNodeColor.RED) {
      boolean isRightFather = node.parent.parent.getRight() == node.parent;

      Function<RBTreeNode<T>, RBTreeNode<T>> uncleGetter =
          isRightFather ? RBTreeNode::getLeft : RBTreeNode::getRight;
      Consumer<RBTreeNode<T>> rotateRightForRightFather =
          isRightFather ? this::rotateRight : this::rotateLeft;
      Consumer<RBTreeNode<T>> rotateLeftForRightFather =
          isRightFather ? this::rotateLeft : this::rotateRight;
  ...
  private void rotate(RBTreeNode<T> node, boolean doRightRotation) {
    Function<RBTreeNode<T>, Boolean> hasSideSon =
        doRightRotation ? RBTreeNode::hasRightSon : RBTreeNode::hasLeftSon;
    Function<RBTreeNode<T>, Boolean> hasOppositeSideSon =
        doRightRotation ? RBTreeNode<T>::hasLeftSon : RBTreeNode<T>::hasRightSon;
    BiConsumer<RBTreeNode<T>, RBTreeNode<T>> sideSonParentSetter =
        doRightRotation ? (s, n) -> s.getRight().setParent(n) : (s, n) -> s.getLeft().setParent(n);
    BiConsumer<RBTreeNode<T>, RBTreeNode<T>> grandsonAsSonSetter =
        doRightRotation ? (n, s) -> n.setLeft(s.getRight()) : (n, s) -> n.setRight(s.getLeft());

    if (null == node || !hasOppositeSideSon.apply(node)) {
      return;
    }
    ...
    BiConsumer<RBTreeNode<T>, RBTreeNode<T>> sideSonSetter =
        doRightRotation ? BTreeNode::setRight : BTreeNode::setLeft;
    sideSonSetter.accept(son,
        node); // узел становится ПРАВЫМ (левым) потомковм ЛЕВОГО (правого) СЫНА
    node.parent = son;
  }
  ...
  private void fixDelete(RBTreeNode<T> node) {
    ...
    while (null != this.getRoot() && node != this.getRoot()
        && node.getColor() == RBTreeNodeColor.BLACK) {
      if (node == node.parent.getLeft()) {
        ...
        if (sibling.getLeft().color == RBTreeNodeColor.BLACK && sibling.getRight().color
            == RBTreeNodeColor.BLACK) {
