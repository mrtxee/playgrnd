 ...
 * Суть сортировки: делить массив на сегменты, выделя каждый раз индекс стены пока все сегменты не
 * окажутся отсортированными.
 */
...
public class QuickSortByArche extends AbstractQuickSort {
  ...
  /**
   * Бежим по массиву, сдвигаем стену только тогда, когда текущее значение массива <= опорного
   * элемента При этом меняем значения стены и текущего элемента местами, чтобы если будет элемент
   * со значением выше опорного элемента, то он сдвигался бы вправо. В конце возвращаем индекс
   * стены, так как он и станет отсортированным элементом.
   */
