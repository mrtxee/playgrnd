...
public abstract class AbstractQuickSort {
  ...
  protected static void swapWithInfo(int[] arr, int i, int j) {
    System.out.printf("%s swapping wall:[%s]%s, current:[%s]%s%n", Arrays.toString(arr), i, arr[i],
        j, arr[j]);
    swap(arr, i, j);
  }

  protected static void swapWithInfo(int[] arr, int i, int j, int pivot) {
    System.out.printf("%s swapping wall:[%s]%s, current:[%s]%s, pivot:%s%n", Arrays.toString(arr),
        i, arr[i], j,
        arr[j], pivot);
    swap(arr, i, j);
  }
